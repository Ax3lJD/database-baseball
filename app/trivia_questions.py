...
def generate_team_performance_question(session_db, asked_question_ids, difficulty='medium'):
    ...
    teams = results  # simplified logic
...
def generate_hof_question(session_db, asked_question_ids, difficulty='medium'):
    ...
    # Query for inducted players in the selected year
    query = text("""
    ...
    try:
        ...
        # Get other players active around the same time as incorrect options
        players_query = text("""
...
def generate_allstar_question(session_db, asked_question_ids, difficulty='medium'):
    ...
    # Query type options based on difficulty
    question_types = []
    ...
    try:
        if question_type == "player_in_allstar":
            # Find players who were in the All-Star game that year
            query = text("""
            ...
            # Get other active players from that year who weren't All-Stars
            ...
            # Find which team had the most All-Stars that year
            teams_query = text("""
            ...
            team_names_query = text("""
                ...
                WHERE teamID IN :team_ids AND yearID = :year;
            """)
...
def generate_worldseries_question(session_db, asked_question_ids, difficulty='medium'):
    ...
    try:
        # Find World Series winner for the year
        ws_query = text("""
        ...
        # Get team names for all teams that year
        teams_query = text("""
            SELECT teamID, team_name FROM teams
            WHERE yearID = :year
            ORDER BY RAND()
        ...
        # Make sure winner is in the options
        if winner_id not in team_name_map:
            # Get winner's name if not in the random selection
        ...
        # Select options including the winner
        options = [(winner_id, team_name_map[winner_id])]

        # Add other teams that aren't the winner
